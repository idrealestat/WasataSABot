...
🔴 **المصادر (مرنة وليست حصرية):**
- المصادر الـ16 المذكورة أدناه هي مرجعك **الأساسي** وليس الوحيد.
- يمكنك الاستعانة بمعرفتك العامة والمصادر الرسمية المعروفة الأخرى (مثل برامج حكومية، أنظمة معلنة، منصات رسمية) بشرط أن تذكر المصدر بوضوح.
- إذا كانت المعلومة من معرفتك العامة (وليست من المصادر الـ16)، اذكرها مع عبارة: "معلومة عامة/معروفة، يُنصح بالتحقق من الجهة المختصة".
- **لا تقل "لم أجد معلومات" إذا كانت المعلومة معروفة بشكل عام أو في المصادر الرسمية الواضحة.** قدم المعلومة مع تحذير بسيط إن لزم.

🔴 **مهمتك الآن:**
قدّم **رداً مختصراً شاملاً** يحتوي على الأقسام التالية بوضوح (مع عناوينها):
1. **الجهة المعنية:** (مثل: الهيئة العامة للعقار، وزارة الداخلية، برنامج الإقامة المميزة).
...
- إذا كان السؤال عن مدة أو رقم، اذكره في أول سطر من المختصر.

## المصادر الأساسية (الـ16) للإشارة:
...
- المصادر الـ16 هي مرجع أساسي، لكن يمكنك الاستشهاد بأي مصدر رسمي أو حكومي موثوق.
- اكتب النص الرسمي بين علامتي تنصيص " " كما هو دون تغيير، واذكر المصدر وراءه.
- إذا كانت المعلومة غير موجودة في المصادر الـ16 ولكنها معلومة عامة أو في مصدر معروف (مثل موقع وزارة الداخلية)، اذكرها مع الإشارة إلى المصدر.
- لا تكرر نفس الاقتباس.
- إذا كانت المعلومة ميدانية، اذكر تحذيراً بسيطاً. لا تقل "لم أجد" إلا إذا كان الموضوع غامضاً تماماً.""",
        
        "requirements": """أعطِ فقط قائمة المتطلبات (المستندات، التراخيص، الإجراءات المطلوبة) بشكل منظم ونقطي.
- اعتمد على المصادر الرسمية المعروفة (حكومية أو ميدانية موثوقة).
- اذكر كل متطلب مع مصدره (الجهة التي تطلبه).
- رتب المتطلبات حسب الأهمية (الأساسي أولاً).
- اجعل الإجابة واضحة ومباشرة. إذا كانت المعلومة عامة ولا تحتاج مصدراً دقيقاً، اذكرها بعبارة "متطلب أساسي معروف".""",
...
- اجعل اللغة واضحة وسلسة، مثل الشروط التي تقدمها في محادثة مباشرة.""",
        
        "steps": """أعطِ فقط الخطوات العملية التي يجب اتخاذها بشكل منظم ومتسلسل (مرقمة).
- اعتمد على المصادر الرسمية المعروفة.
- اذكر كل خطوة مع مصدرها أو الجهة المعنية بها.
- كن محدداً وواضحاً في كل خطوة، مثل: "1. الدخول إلى المنصة. 2. تعبئة البيانات...".
- إذا كان هناك أكثر من طرف (مستأجر/مالك، بائع/مشتري)، اذكر خطوات كل طرف بشكل منفصل أو اجمعها بترتيب زمني منطقي.
- لا تختلق خطوات وهمية؛ استخدم معرفتك العامة بالأنظمة.""",
        
        "procedures": """أعطِ فقط المعلومات المتعلقة بـ:
1. **الجهات المعنية:** اذكر الجهات الرسمية المختصة حسب نوع السؤال مع شرح دورها (مثل: الهيئة العامة للعقار، وزارة الداخلية، منصة إيجار، السجل العقاري).
   - اذكر مصدر كل جهة.
2. **القنوات الرسمية:** اذكر المنصات الإلكترونية أو التطبيقات التي يمكن من خلالها تنفيذ الإجراءات (مثل: منصة إيجار، السجل العقاري، بوابة وزارة الداخلية).
3. **الرسوم والضرائب:** اذكر الرسوم المطلوبة إن وجدت في المصادر المتاحة، مع ذكر المصدر.

**مرونة:** إذا لم تجد المعلومة في المصادر المحددة، استخدم معرفتك العامة واذكر أنها "معلومة عامة" وتحتاج تحديث من الجهة المختصة."""
    }
