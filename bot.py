...
# ======================= البرومبت الكامل (مع 16 مصدراً وتنسيق Bold) =======================
BASE_SYSTEM_PROMPT = """
...
إذا كان السؤال يتطلب إجراءات أو خطوات (مثل: كيف، طريقة، إجراءات، خطوات، متطلبات، شروط)، يجب عرض العناصر التالية في التفصيل:
1. الشروط
2. الإجراءات
...
"""
    ...
    # ====== أزرار اختيار نوع العقد (وساطة / إيجار) ======
        ...
        if last_q:
            ...
            detailed_prompt = f"المستخدم يسأل عن عقد وساطة. ابحث في المصادر الـ16 (باستثناء منصة إيجار)، مع التركيز على نظام الوساطة العقارية (م/130)، وقدم الإجابة كاملة مع الشروط والإجراءات والخطوات."
        ...
        if last_q:
            ...
            detailed_prompt = f"المستخدم يسأل عن عقد إيجار. ابحث في منصة إيجار، والهيئة العامة للعقار، والمصادر الميدانية، وقدم الإجابة كاملة مع الشروط والإجراءات والخطوات."
            reply = get_ai_response(detailed_prompt)
            if FOOTER.strip() not in reply.strip():
                reply = reply + FOOTER
            await query.edit_message_text(reply, parse_mode=ParseMode.MARKDOWN)
    ...
    # ====== أزرار التقييم (نعم / لا) ======
        ...
        if last_q:
            ...
            # التحقق مما إذا كان هناك سياق إضافي (هل هناك سؤال تالي)
            # إرسال رسالة متابعة
        ...
        if last_q:
            ...
            new_prompt = f"أعد صياغة الإجابة على: {last_q} مع التأكد من المصادر الـ16 وعرض جميع العناصر المطلوبة."
