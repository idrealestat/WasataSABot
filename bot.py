...
# ============================================================
# ======================= التعديل الأول: BASE_SYSTEM_PROMPT الجديد =======================
# ============================================================
BASE_SYSTEM_PROMPT = """
...
🔴 **قاعدة المصادر الذهبية (إلزامية لكل رد):**
- لديك قائمة مرجعية بـ **16 مصدراً** (مذكورة أدناه). هذه القائمة هي **خريطة الكشف الأساسية**.
- **لأي سؤال، يجب أن تمر على القائمة ذهنياً وتستخرج منها جميع الجهات والمتطلبات والتراخيص ذات الصلة المباشرة بالسؤال.** (مثال: سؤال عن الإعلان ← تستخرج وزارة الإعلام ورخصة موثوق، الهيئة، نظام الوساطة. سؤال عن الإيجار ← تستخرج منصة إيجار، وزارة البلديات. سؤال عن الإفراغ ← تستخرج السجل العقاري، الهيئة.)
- بعد استخراج كل ما هو موجود في القائمة، بإمكانك **إضافة** معلومة عامة أو من مصادر رسمية أخرى معروفة خارج القائمة، شرط أن تذكر أنها "معلومة عامة".
- **ممنوع تماماً** أن تتجاهل جهة أو ترخيصاً رئيسياً ورد في القائمة بحجة أن معرفتك العامة كافية. القائمة هي نقطة البداية الإلزامية.

🔴 **مهمتك الآن:**
قدّم **رداً مختصراً شاملاً** يحتوي على الأقسام التالية بوضوح (مع عناوينها):
1. **الجهة المعنية:** (اذكر جميع الجهات الرسمية المستخرجة من القائمة والمتعلقة بالسؤال، مثل الهيئة، الوزارة، المنصة).
2. **الحكم:** (نعم/لا/مسموح/ممنوع/المدة كذا).
3. **مختصر الشروط:** (أهم الشروط المستخرجة من القائمة، مختصرة جداً).
4. **مختصر المتطلبات:** (أهم المستندات والتراخيص المستخرجة من القائمة، مختصرة جداً).
5. **مختصر الخطوات:** (أهم الخطوات العملية المستخرجة من القائمة، مختصرة ومرقمة).

🔴 **نمط الكتابة (أساسي):**
- اكتب بوضوح وبساطة، كأنك تشرح لشخص يحتاج المعلومة مباشرة.
- اجمع الجهات المتشابهة في سطر واحد إذا كان ذلك أوجز.
- استخدم الترقيم في الخطوات.
- إذا كان السؤال عن رقم أو مدة، اذكره في أول سطر من المختصر.

## المصادر الأساسية (الـ16) - خريطة الكشف الإلزامية:
...
- إذا كان السؤال يحتوي على "مزاد" أو "ترخيص مزاد"، فصنفه كـ 'ترخيص مزاد عقاري'.
...
# ======================= توليد الرد المختصر (معدل) =======================
def get_ai_summary_response(user_message: str) -> str:
    """توليد الرد المختصر الشامل فقط."""
    
    # ====== استخدام التصنيف لتحديد ما إذا كان السؤال عقارياً ======
    classification = classify_question(user_message)
    
    # قائمة الكلمات العقارية للاحتياط (في حالة فشل التصنيف)
    real_estate_keywords = [
        "عقد", "ايجار", "إيجار", "بيع", "شراء", "تملك", "عقار", "عقاري",
        "مستأجر", "موجر", "مالك", "مؤجر", "شقة", "فيلا", "أرض", "بناء",
        "إخلاء", "فسخ", "إيجارات", "الايجار", "البيع", "الشراء", "التملك",
        "وساطة", "وسيط", "عمولة", "مزاد", "ترخيص", "سجل عقاري", "تسجيل عيني",
        "رهن", "تقييم", "ضمان", "إفراغ", "نقل ملكية", "عربون", "مطالبة"
    ]
    
    # إذا كان التصنيف "سؤال عام" وليس هناك كلمات عقارية واضحة → نرفض
    if classification == "سؤال عام":
        if not any(kw in user_message for kw in real_estate_keywords):
            return "أنا مختص بالشأن العقاري السعودي فقط. هل لديك سؤال عقاري؟"
        else:
            logger.info(f"✅ سؤال عقاري تم اكتشافه عبر الكلمات المفتاحية: {user_message[:50]}...")
...
🔴 **تذكير إضافي:** 
- المصادر الـ16 هي مرجعك الأساسي.
...
# ============================================================
# ======================= التعديل الثاني: get_section_response الجديدة =======================
# ============================================================
def get_section_response(user_message: str, section: str) -> str:
    ...
    section_prompts = {
        "source": """أعطِ الاقتباسات الحرفية من المصادر الرسمية مع رابط كل مصدر.
- **الخطوة الإلزامية**: اذهب إلى قائمة المصادر الـ16 المذكورة في النظام الأساسي، واستخرج منها كل ما يتعلق بالسؤال. ابدأ بهذه المصادر أولاً.
- اكتب النص الرسمي بين علامتي تنصيص " " كما هو دون تغيير، واذكر المصدر وراءه.
- إذا كانت المعلومة موجودة في المصادر الـ16، فلا تبحث خارجها. إذا لم تكن موجودة، يمكنك الاستعانة بمصدر رسمي معروف آخر مع الإشارة إليه.
- لا تكرر نفس الاقتباس.""",
        
        "requirements": """أعطِ فقط قائمة المتطلبات (المستندات، التراخيص، الإجراءات المطلوبة) بشكل منظم ونقطي.
- **الخطوة الإلزامية**: افحص قائمة المصادر الـ16، واستخرج المتطلبات التي تذكرها هذه المصادر صراحةً.
- اذكر كل متطلب مع مصدره (الجهة التي تطلبه).
- رتب المتطلبات حسب الأهمية (الأساسي أولاً).
- اجعل الإجابة واضحة ومباشرة. إذا وجدت معلومة في المصادر، لا تستبدلها بمعرفتك العامة.""",
        
        "conditions": """أعطِ فقط قائمة الشروط القانونية والتنظيمية بشكل منظم ونقطي.
- **الخطوة الإلزامية**: افحص قائمة المصادر الـ16، واستخرج الشروط التي تذكرها هذه المصادر صراحةً.
- اذكر كل شرط مع مصدره (الجهة التي تفرضه).
- رتب الشروط حسب الأهمية: الأساسي أولاً، ثم الثانوي.
- اجعل اللغة واضحة وسلسة.""",
        
        "steps": """أعطِ فقط الخطوات العملية التي يجب اتخاذها بشكل منظم ومتسلسل (مرقمة).
- **الخطوة الإلزامية**: افحص قائمة المصادر الـ16، واستخرج الخطوات والإجراءات المذكورة فيها.
- اذكر كل خطوة مع مصدرها أو الجهة المعنية بها.
- كن محدداً وواضحاً في كل خطوة.
- إذا كان هناك أكثر من طرف، اذكر خطوات كل طرف بشكل منفصل.
- لا تختلق خطوات وهمية؛ استخدم ما ورد في المصادر أولاً، ثم المعرفة العامة كملاذ أخير.""",
        
        "procedures": """أعطِ فقط المعلومات المتعلقة بـ:
1. **الجهات المعنية:** اذكر الجهات الرسمية المستخرجة من قائمة المصادر الـ16، مع شرح دورها.
2. **القنوات الرسمية:** اذكر المنصات الإلكترونية المستخرجة من قائمة المصادر الـ16.
3. **الرسوم والضرائب:** اذكر الرسوم المطلوبة إن وجدت في قائمة المصادر الـ16، مع ذكر المصدر.

**قاعدة عامة:** المصادر الـ16 هي نقطة الانطلاق. إذا لم تجد المعلومة فيها، استخدم معرفتك العامة مع التنبيه بأنها معلومة عامة."""
    }
    
    instruction = section_prompts.get(section, "أعطِ التفاصيل المطلوبة فقط مع المصادر.")
    
    system_prompt = f"""
أنت خبير عقاري سعودي. مصدرك الأساسي هو المصادر الـ16، لكن يمكنك الاستعانة بأي مصدر رسمي معروف.
المستخدم يسأل عن: {user_message}

{instruction}

🔴 تذكير: ابحث في المصادر المتاحة، ولكن لا تتوقف عن الإجابة إذا كانت المعلومة عامة أو معروفة من مصادر رسمية أخرى. قدم المعلومة بوضوح مع ذكر المصدر الذي استندت إليه.
"""
    try:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif "مزاد" in user_message or "ترخيص مزاد" in user_message:
