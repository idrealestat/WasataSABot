...
def search_youtube_general(query, api_key, max_results=5, order='date'):
    try:
        ...
        request = youtube.search().list(
            ...
            q=query,
            type='video',
            maxResults=max_results * 3,  # جلب 3 أضعاف للتصفية
            order=order,
            regionCode='SA'
        )
        response = request.execute()
        ...
        for item in response['items']:
            ...
            # قبول الفيديو إذا كان يحتوي على كلمتين على الأقل من القائمة
...
def search_youtube(query, api_key, max_results=5):
    ...
    # تحسين الاستعلام بناءً على نوع السؤال
    if "عقد وساطة" in query or "عقد وساطه" in query:
        ...
        search_query = f"{query} إجراءات عقارية سعودية شرح"
    else:
        search_query = f"{query} عقار سعودي شرح"
...
🔴 **نظام التسجيل العيني للعقار (إضافة مهمة):**

التسجيل العيني هو نظام قانوني معمول به في المملكة العربية السعودية بموجب "نظام التسجيل العيني للعقار" الصادر بالمرسوم الملكي رقم (م/22) وتاريخ 1443/02/02هـ.

**ماهو التسجيل العيني؟**
هو إجراء يقوم به مالك العقار (أو من ينوب عنه) لتسجيل عقاره في السجل العقاري، حيث يُسجَّل العقار بوصفه وحدة مادية محددة الحدود والمعالم، مع بيان جميع الحقوق المتعلقة به (ملكية، رهن، إيجار، وغيرها).

**الخطوات الأساسية للتسجيل العيني:**
1. **تقديم طلب التسجيل** عبر منصة السجل العقاري (https://rer.sa).
2. **إرفاق المستندات المطلوبة** (صك الملكية، الهوية، المخططات، إلخ).
3. **الفحص والمراجعة** من قبل مسجل عقاري معتمد.
4. **النشر والإعلان** عن طلب التسجيل (لمدة 30 يوماً).
5. **الاعتراضات** (إن وجدت) والبت فيها.
6. **التسجيل النهائي** وإصدار الصك العيني.

**مصادر رسمية:**
- نظام التسجيل العيني للعقار: https://laws.boe.gov.sa
- منصة السجل العقاري: https://rer.sa
- الهيئة العامة للعقار: https://rega.gov.sa
- دليل إجراءات التسجيل العيني: https://rega.gov.sa/rega-services/eservices/التسجيل-العيني-الأول/

🔴 **عقد الوساطة العقارية (إجراءات عملية):**

عند سؤال المستخدم عن كيفية إعداد عقد وساطة، يجب تقديم المعلومات التالية:

**مكونات عقد الوساطة العقارية الأساسية:**
1. **أطراف العقد:** المالك (الموكل) والوسيط العقاري (الوكيل).
2. **موضوع العقد:** تحديد العقار محل الوساطة (وصف دقيق، عنوان، حدود).
3. **مدة العقد:** فترة سريان العقد (عادة 3-6 أشهر قابلة للتجديد).
4. **العمولة:** تحديد نسبة العمولة المستحقة للوسيط (عادة 1-2.5% من قيمة البيع أو الإيجار).
5. **حقوق والتزامات الأطراف:** مسئوليات الوسيط في التسويق، والمالك في التعاون وتقديم المستندات.
6. **آلية الإنهاء:** شروط فسخ العقد من قبل أي من الطرفين.
7. **التوقيع والتوثيق:** يجب توثيق العقد في منصة "إيجار" أو السجل العقاري.

**المراجع:**
- نظام الوساطة العقارية (م/130): https://rega.gov.sa
- نموذج عقد وساطة معتمد من الهيئة: https://rega.gov.sa
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== الحصول على الرد من الذكاء الاصطناعي أولاً =====
    ...
    # ===== البحث عن فيديوهات يوتيوب كمكمل =====
