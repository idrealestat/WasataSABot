...
class YouTubeService:
    ...
    # ======== التعديل: إضافة "عقد وساطة" و "عقد وساطه" بشكل صريح ========
    TOPIC_QUERIES = {
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            ...
            # ======== رسائل خطأ محددة للذكاء الاصطناعي ========
            try:
                ai_reply = await ai_task
            except Exception as e:
                error_msg = str(e).lower()
                if "groq" in error_msg:
                    await update.message.reply_text("⚠️ **خطأ في خدمة Groq:** تعذر الاتصال بخادم Groq. جاري محاولة استخدام الخدمات الاحتياطية...")
                elif "openrouter" in error_msg:
                    await update.message.reply_text("⚠️ **خطأ في خدمة OpenRouter:** تعذر الاتصال بخادم OpenRouter. جاري محاولة استخدام الخدمات الاحتياطية...")
                elif "gemini" in error_msg or "google" in error_msg:
                    await update.message.reply_text("⚠️ **خطأ في خدمة Google Gemini:** تعذر الاتصال بخادم Gemini. يرجى المحاولة لاحقاً.")
                elif "timeout" in error_msg or "timed out" in error_msg:
                    await update.message.reply_text("⏰ **انتهت مهلة الاتصال:** استغرق الطلب وقتاً طويلاً. يرجى المحاولة مرة أخرى.")
                elif "quota" in error_msg or "rate limit" in error_msg:
                    await update.message.reply_text("📊 **تم استنفاذ الحصة اليومية:** تم تجاوز الحد المسموح به من الطلبات. يرجى المحاولة غداً.")
                else:
                    await update.message.reply_text(f"❌ **حدث خطأ غير معروف:** {str(e)}. يرجى المحاولة لاحقاً.")
                ai_reply = "❌ عذراً، حدث خطأ في معالجة طلبك. يرجى المحاولة لاحقاً."
                logger.error(f"❌ فشل توليد الرد من AI: {e}")
            
            # ======== رسائل خطأ محددة لـ YouTube ========
            if youtube_task:
                try:
                    ...
                    if "quota" in error_msg or "rate limit" in error_msg:
                        await update.message.reply_text("📊 **تم استنفاذ حصة YouTube API:** لا يمكن إجراء المزيد من عمليات البحث اليوم.")
                    elif "403" in error_msg or "forbidden" in error_msg:
                        await update.message.reply_text("🚫 **خطأ في صلاحيات YouTube API:** يرجى التحقق من مفتاح API.")
                    elif "404" in error_msg or "not found" in error_msg:
                        await update.message.reply_text("🔍 **لم يتم العثور على فيديوهات:** لا توجد نتائج لهذا البحث.")
                    else:
                        await update.message.reply_text(f"⚠️ **خطأ في YouTube:** {str(e)}")
                    youtube_results = []
        ...
        # ======== رسالة عند عدم العثور على فيديوهات (محددة) ========
        youtube_reply = ""
        if youtube_results and len(youtube_results) > 0:
            ...
            # هذه هي رسالة "عدم العثور على فيديوهات" المطلوبة
