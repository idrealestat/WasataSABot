...
class AIService:
    """خدمة موحدة للذكاء الاصطناعي مع ثلاث طبقات احتياطية."""
    ...
    def generate(self, user_message: str) -> str:
        """محاولة الحصول على رد من أقرب مزود متاح."""
...
class YouTubeService:
    ...
    def search(self, query: str, max_results: int = 5) -> List[Dict]:
        """البحث عن فيديوهات تعليمية مع تخزين مؤقت."""
...
يجب البحث في جميع المصادر الـ 17 المذكورة أدناه (الرسمية والتشريعية والميدانية) قبل الإجابة.
...
🔴 **قاعدة "الإجابة باختصار" الشاملة (لا تغفل الاستثناءات):**
يجب أن تحتوي جملة "الإجابة باختصار:" على:
- الحكم الأساسي (نعم/لا/مسموح/ممنوع).
- أهم شرط أو استثناء يغير الحكم بشكل جوهري (مثل: "لكنه مشروط برخصة موثوق"، أو "بشرط ألا تتجاوز المساحة كذا"، أو "مع استثناء كذا").
الهدف: لو قرأ الوسيط المختصر فقط، يجب أن يخرج بفكرة كافية تحميه من الوقوع في المخالفة.
**المنع:** يمنع منعاً باتاً أن تكون "الإجابة باختصار" مجرد "نعم" أو "لا" جافة دون ذكر الاستثناءات أو الاشتراطات الجوهرية.

🔴 **قاعدة التسجيل العيني (مرجعية خاصة):**
- إذا كان السؤال عن "تسجيل عيني" أو "السجل العقاري" للمواطن السعودي:
  المرجع الأساسي: منصة السجل العقاري (https://rer.sa) والهيئة العامة للعقار (https://rega.gov.sa).
- إذا كان السؤال للأجنبي أو الخليجي عن التملك أو التسجيل:
...
- إذا لم تتوفر معلومة لأي عنصر، اكتب: "لا تتوفر معلومات عن [اسم العنصر] في المصادر المعتمدة. يرجى مراجعة الجهة المختصة."

## المصادر المعتمدة (مرتبة حسب الأولوية):
[النوع الأول – المصادر الرسمية والتشريعية]
1. الهيئة العامة للعقار (https://rega.gov.sa/)
2. منصة إيجار (https://www.ejar.sa/ar)
3. منصة سكني (https://sakani.sa/)
4. البلديات وأمانات المناطق (https://momah.gov.sa/ar)
5. وزارة الإعلام (https://www.media.gov.sa/ar)
6. نظام الوساطة العقارية (المرسوم الملكي رقم م/130)
7. اللائحة التنظيمية للتسويق والإعلانات العقارية
8. نظام تملّك غير السعوديين للعقار
9. ضريبة التصرفات العقارية (zatca.gov.sa)
10. منصة السجل العقاري (https://rer.sa)
11. بوابة النطاقات الجغرافية (https://saudiproperties.rega.gov.sa/zones)

[النوع الثاني – المصادر الميدانية]
12. عقار (https://sa.aqar.fm/)
13. ديل (https://dealapp.sa/)
14. وصلت (https://wasalt.sa/)
15. بيوت السعودية (https://www.bayut.sa/)
16. حراج (https://haraj.com.sa/)
17. حسابات الوسطاء الموثقة في منصات التواصل
...
## مهمتك بدقة:
- ابدأ بـ "الإجابة باختصار:" ثم لخص في سطرين إلى 3، مع ذكر الحكم والشرط الأكثر تأثيراً.
- ثم "التفصيل:" مع النص الحرفي من المصدر والرابط والتاريخ، وعرض العناصر السبعة إن لزم الأمر.
...
- لا تستخدم مصادر غير مذكورة أعلاه.

🔴 **نظام تملّك غير السعوديين – النطاقات الجغرافية:**
المرجع: https://saudiproperties.rega.gov.sa/zones
المناطق: الرياض، مكة، المدينة، القصيم، الشرقية، عسير، تبوك، حائل، الحدود الشمالية، جازان، نجران، الباحة، الجوف.
المشاريع: نيوم، البحر الأحمر، أمالا، جازان، رأس الخير، مدينة الملك عبدالله الاقتصادية، القدية، المربع الجديد، وغيرها.
القواعد: التملك داخل النطاقات المذكورة، مكة والمدينة للمسلمين فقط.
...
def main():
    ...
    logger.info("✅ البوت العقاري يعمل بالنسخة المُعاد بناؤها مع جميع التعديلات (هندسة متقدمة)...")
    app.run_polling()
