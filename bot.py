...
# قائمة القنوات الثانوية (تم إلغاؤها لتوفير الحصة)
...
def get_channel_id_from_handle_cached(handle, api_key):
    """نفس الدالة لكن مع تخزين مؤقت لتجنب الطلبات المتكررة."""
    try:
...
def search_youtube(query, api_key, max_results=3):
    ...
    for handle in OFFICIAL_CHANNELS_HANDLES:
        ...
        if channel_id:
            ...
            # إذا فشل استخراج ID بسبب الحصة، نخرج من الحلقة ونتجه للبحث العام
...
**أولاً: المناطق المذكورة في النطاقات الجغرافية (13 منطقة):**
...
**مدينة الرياض (9 مشاريع):** القدية، المربع الجديد، المسار الرياضي ومنطقة الفنون، بوابة الدرعية، حديقة الملك سلمان، سدرة، مركز الملك عبدالله المالي (كافد)، مطار الملك سلمان الدولي، مواقع التطوير الموجه للنقل العام.
**محافظة جدة:** أبتاون، العروس، وسط جدة، ومناطق التطوير (1) إلى (55).
**مدينة مكة المكرمة (12 مشروع):** أبراج مكة، المنار، برج أجياد، بوابة الملك سلمان، تلال فيليج، جبل عمر، ذاخر مكة، ضاحية سمو، مسار، مكة (منطقة 1، 2، 3).
**المدينة المنورة (10 مشاريع):** الغرة، المدينة المنورة (منطقة 1، 2)، المهوى، دار الهجرة، داون تاون المدينة، ديار المقر، رؤى المدينة، مدينة المعرفة الاقتصادية، مشراف.
**محافظة العلا (17 منطقة):** العلا – منطقة (1) إلى (17).

**ثالثاً: القواعد الأساسية:**
- التملك متاح لغير السعوديين داخل النطاقات الجغرافية المذكورة أعلاه، بشرط أن يكون العقار مسجلاً تسجيلاً عينياً في السجل العقاري.
- مكة المكرمة والمدينة المنورة: التملك فيهما يقتصر على المسلمين فقط.
- الرياض وجدة: التملك متاح في المناطق المذكورة أعلاه فقط (وليس كامل المدينة).
...
- بوابة "عقارات السعودية" لتقديم الطلبات: https://saudiproperties.rega.gov.sa
...
async def zones_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    أمر عرض النطاقات الجغرافية لتملك غير السعوديين.
    """
    user = update.effective_user
    # تسجيل أن المستخدم استخدم هذا الأمر (مفيد للإحصائيات)
...
📢 **ملاحظة مهمة:**
سيتم إضافة تحديثات جديدة للنطاقات والخدمات قريباً. تابع المنصة الرسمية للمستجدات.
...
"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📞 الدعم والاتصال", url="https://saudiproperties.rega.gov.sa")]
    ]
...
def main():
    ...
    # أمر النطاقات الجغرافية (لجميع المستخدمين)
